...
#    autosave => 1
  ...
  if($file_to_retrieve){
    ...
    $self->{ua}->get('http://xml.yandex.ru'); # дл€ куков
  }
...
sub _step1 {
  ...
  if($resp2->content =~ m{class="loginlist visible"}){ # список доступных логинов
    # создаем валидный (дл€ яндекса) логин 
    $login = _parse_yandex_logins($resp2->content);
    $form1 = HTML::Form->parse($resp2);
    # TODO: им€ и фамили€ заполн€етс€ не в той кодировке
    $form1->value(iname => $self->{iname});
    $form1->value(fname => $self->{fname});
...
sub get_captcha {
  ...
  unless($src){
    ...
    $self->{ua}->get($src); # Ё“ќ ќ„≈Ќ№ ¬ј∆Ќќ, »Ќј„≈ –≈√»—“–ј÷»я Ќ≈ ѕ–ќ…ƒ≈“

  # сохран€ем объект в файле $rand_file_name
...
sub send_captcha {
  ...
  if($html =~ /¬ы неправильно ввели контрольные цифры./){
    unlink $self->{store} if $self->{store};
    return 0;
  }
  if($html =~ /ќшибка обработки запроса/){
    die "bad request (?):\n", Dumper ($req);
  }
  if($html =~ /\(Cookies\)/){
    die $req->as_string, "\nнеобходимо включить куки (Cookies)";
  }
  unless($html =~ /ѕоздравл€ем/){
    open my $w, '>resp3.html';
    print $w $html;
    close $w;
    die "неизвестна€ ошибка";
  }
  
  $self->{ua}->get('http://passport.yandex.ru/passport?mode=logout'); # дл€ очистки куков
  unlink $self->{store} if $self->{store};
...
sub my_lc {
  my $self = shift;
  $self =~ tr/јЅ¬√ƒ≈®∆«»… ЋћЌќѕ–—“”‘’÷„Ўў№џЏЁёя/абвгдеЄжзийклмнопрстуфхцчшщьыъэю€/;
  return $self;
}
...
sub _create_login($$) {
  ...
  my $line = $iname . " " . $fname;
  my $login = my_lc( $line );
  $login =~ tr/ абвгдеЄжзийклмнопрстуфхцчшщэьъю€/.abvgdeejzijklmnoprstufhc4wwe77uy/;
  $login =~ s{y}{ja};
#  $login .= ".dr";
  return $login; 
}
...
sub fetch_person {
  ...
  return($iname, $fname);
} # можно сделать случайные им€-фамилию из файла как здесь
